...
class Lattice2DEnv(gym.Env):
    ...
    def __init__(self, seq, collision_penalty=-2, trap_penalty=0.5):
        ...
        self.actions = [1] * (len(self.seq) - 1)
        #self.actions = [1] 
        res_0 = Residue(0, self.seq[0], (0,0))
        self.state = OrderedDict( { 0 : res_0 })
        self.build_state(self.actions)
        self.collisions = 0
        self.trapped = 0
    ...
    def _compute_state(self, state, actions):
        #state = OrderedDict({(0, 0) : self.seq[0]})
        state = OrderedDict({0 : Residue(0, self.seq[0], (0,0) )})
        # if (actions < (len(self.seq) - 1)):
        ...
        # if (len(state) < (len(self.seq))):
        #     print(actions)
        #     print(state)
        #     print("error at state " + str(len(actions)))
        #     exit()
 
        return state
 
    def build_state(self, actions):
        self.actions = actions
        self.state = self._compute_state(self.state, actions)
        self.grid = self._draw_grid(self.state)
    ...
    def step(self, action, idx):
        ...
        self.build_state(self.actions)
        (x,y) = self.state[list(self.state.keys())[idx]].coords
        # Get all adjacent coords and next move based on action
        adj_coords = self._get_adjacent_coords((x, y))
        next_move = adj_coords[action]
        # Detects for collision or traps in the given coordinate
        all_coords = [res.coords for i, res in self.state.items()]
        if set(adj_coords.values()).issubset( all_coords ):
            #logger.warn('Your agent was trapped! Ending the episode.')
            self.trapped += 1
            is_trapped = True
        elif next_move in all_coords:
            self.collisions += 1
            collision = True
        ...
        #done = (((len(self.seq) - 1) == (len(self.actions))) or has_collision)
        done = (has_collision)
    ...
    def render(self, mode='human'):
        ...
        #self.render_server.render( points, self.last_position)
        # Decode if possible
        desc.tolist()
        try:
    ...
    def _try_compute_reward(self, state, is_trapped, collision, done):
        ...
        actual_trap_penalty = len(set(self.actions)) * 0.07
        ...
        reward = state_reward - collision_penalty * 20 - actual_trap_penalty + (-1) * len(state.items()) * 0.05
        return reward

    def _compute_reward(self, is_trapped, collision, done):
        return self._try_compute_reward(self.state, is_trapped, collision, done )
